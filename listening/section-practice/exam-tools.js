(function () {
  ...
  function bindEvents() {
    document.addEventListener("mouseup", () => setTimeout(showToolbarFromSelection, 0));
    ...
    document.addEventListener("selectionchange", () => {
      ...
      if (selectionInsideInput(sel)) return;
  ...
  function showToolbarFromSelection() {
    ...
    const left = Math.max(12, rect.left + scrollX);
  ...
  function saveNote() {
    ...
    notes.push({ selectedText, noteText, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) });
    renderNotes();
